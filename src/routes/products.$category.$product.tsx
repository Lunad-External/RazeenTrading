...
import { ProductCard, WhatsAppButton } from "@/components/catalogue-ui";
import { categories, getProduct, products } from "@/lib/catalogue";
export const Route = createFileRoute("/products/$category/$product")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: loaderData ? `${loaderData.name} | Razeen UAE` : "Product unavailable | Razeen" },
      {
        name: "description",
        content: loaderData?.description ?? "Product information from Razeen.",
      },
      {
        property: "og:title",
        content: loaderData ? `${loaderData.name} | Razeen UAE` : "Product unavailable | Razeen",
      },
      {
...
function ProductPage() {
  ...
  const categoryName =
    categories.find((item) => item.slug === product.category)?.name ?? "Products";
    ...
    <>
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-2 lg:items-center">
          ...
          <div>
            ...
            <p className="mt-6 text-sm font-semibold text-muted-foreground">
              Pricing is available by enquiry.
            </p>
      ...
      <section className="section-space bg-surface">
        <div className="site-container grid gap-10 lg:grid-cols-2">
          <div>
            <h2 className="text-2xl font-bold text-primary">Key specifications</h2>
            <ul className="mt-6 grid gap-4">
              {product.specs.map((spec) => (
                <li key={spec} className="flex gap-3">
                  <Check className="mt-0.5 size-5 shrink-0 text-primary-light" />
                  <span>{spec}</span>
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <p className="mt-6 leading-7 text-muted-foreground">
              Suitable for professional construction, installation, repair and maintenance
              requirements. Confirm project-specific suitability with the Razeen team when
              requesting a quote.
            </p>
          </div>
        </div>
